...
export async function GET(request: NextRequest) {
	try {
		...
		const { data: subscription, error: subError } = await supabase
			...
			.select(
				'tier, websites_limit, trial_expires_at, trial_websites_requested',
			)
			.eq('user_id', user.id)
			.single();
		...
		return NextResponse.json({
			...
			websites_remaining: Math.max(
				0,
				subscription.websites_limit - websitesUsed,
			),
			trial_expires_at: subscription.trial_expires_at,
			trial_websites_requested: subscription.trial_websites_requested,
			is_trial_expired: isTrialExpired,
			can_scan:
				!isTrialExpired && websitesUsed < subscription.websites_limit,
		});
	} catch (error) {
